...
export function AppSidebar({
  ...
  const { role, employeeEstablishment } = useAuth()
  ...
  const { data: plan, isLoading: planIsLoading } = useQuery({
    ...
    enabled: !isEmployee && !!currentSubscription?.plan.id,
  })

  const totalEmployees = employees?.length ?? 0

  const displayName = isEmployee
    ? employeeEstablishment?.name
    : establishment?.name
  ...
  const employeeItems = [
    ...
    },
    {
      ...
      icon: Activity,
      isActive: pathname === "/app/my-dashboard",
    },
  ]
  ...
  return (
    <Sidebar collapsible="offcanvas" {...props}>
      ...
      <SidebarContent>
        <NavMain
          items={isEmployee ? employeeItems : partnerItems}
        />
