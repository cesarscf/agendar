    ...
    typedApp.post(
      ...
      async (request, reply) => {
        ...
        const [createdPackage] = await db
          ...
          .values({
            ...data,
            price: centsToReais(data.price),
            commission: commissionToDb(data.commission),
            establishmentId,
          })
          .returning({
            id: packages.id,
        ...
        return reply.status(201).send({
          ...createdPackage,
          price: reaisToCents(createdPackage.price),
          commission: commissionToFront(createdPackage.commission),
        })
      }
    )
  })
